...
['zhanqiao','栈桥','海景','青岛经典海岸地标，清晨和傍晚更从容。',120.3193,36.061736,50,'平坦，临海风大','全年'],
...
function applyRoute(id){selected=[...routes.find(r=>r.id===id).ids];if(document.body.classList.contains('home-page')){const data={p:selected,d:'',t:'09:00',s:'青岛站',m:'smart',v:'normal'},encoded=btoa(unescape(encodeURIComponent(JSON.stringify(data))));location.href=`planner.html#trip=${encoded}`;return}renderPlaces();buildPlan();location.hash='planner';}
...
const placeVisual={
  '海景':{symbol:'海',color:'#08788a'},'博物馆':{symbol:'馆',color:'#6f6655'},'公园':{symbol:'园',color:'#4f785e'},'亲子游乐':{symbol:'乐',color:'#b06b3d'},
  '美食街':{symbol:'食',color:'#a64f44'},'购物':{symbol:'购',color:'#776590'},'城市漫游':{symbol:'城',color:'#476b80'}
};
function renderCategoryChart(){if(!categoryChart)return;const cats=Object.keys(placeVisual),total=places.length;categoryChart.innerHTML=`<div class="category-legend">${cats.map(cat=>{const count=places.filter(p=>p.category===cat).length;return `<span style="--cat-color:${placeVisual[cat].color}"><i></i>${cat} ${count}</span>`}).join('')}</div><div class="category-bar" role="img" aria-label="共 ${total} 个地点，按七类显示数量比例">${cats.map(cat=>{const count=places.filter(p=>p.category===cat).length;return `<span style="--share:${count/total*100}%;--cat-color:${placeVisual[cat].color}" title="${cat}：${count} 个"></span>`}).join('')}</div>`}
function renderFilters(){const cats=['全部',...Object.keys(placeVisual)];filters.innerHTML=cats.map(c=>{const count=c==='全部'?places.length:places.filter(p=>p.category===c).length;return `<button class="filter ${c===activeCategory?'active':''}" data-cat="${c}">${c}<small>${count}</small></button>`}).join('');$$('[data-cat]').forEach(b=>b.onclick=()=>{activeCategory=b.dataset.cat;showAll=false;renderFilters();renderPlaces()});}
function renderPlaces(){let list=activeCategory==='全部'?places:places.filter(p=>p.category===activeCategory);const visible=showAll?list:list.slice(0,12);placeGrid.innerHTML=visible.map(p=>{const info=bookingInfo[p.id],visual=placeVisual[p.category]||{symbol:'点',color:'#08788a'};return `<article class="place-card ${selected.includes(p.id)?'selected':''}"><button class="place-select" data-place="${p.id}" type="button" aria-pressed="${selected.includes(p.id)}" title="${esc(p.summary)}"><span class="place-symbol" style="color:${visual.color};background:${visual.color}16">${visual.symbol}</span><div class="place-title-row"><h3>${p.name}</h3><span>${p.category}</span></div><div class="place-metrics"><span>${p.season}</span><span>${p.duration} 分钟</span><span>${p.slope}</span></div><i class="check">${selected.includes(p.id)?'✓':'+'}</i></button>${info?`<div class="place-booking booking-${info.level}"><span>${info.label}</span><a href="${info.url}" target="_blank" rel="noreferrer">${info.action}</a></div>`:''}</article>`}).join('');morePlaces.hidden=list.length<=12;morePlaces.textContent=showAll?'收起地点':'显示更多地点';$$('[data-place]').forEach(b=>b.onclick=()=>togglePlace(b.dataset.place));selectedCount.textContent=selected.length;}
...
function drawMap(ids){
  const pts=ids.map(byId).filter(Boolean),lines=[];
  for(let x=80;x<=920;x+=120)lines.push(`<line class="map-grid" x1="${x}" y1="55" x2="${x}" y2="535"/>`);
  for(let y=55;y<=535;y+=80)lines.push(`<line class="map-grid" x1="80" y1="${y}" x2="920" y2="${y}"/>`);
  const coast=`<path class="map-coast" d="M60 500 C180 480 205 402 300 430 S420 515 535 427 S700 344 770 250 S875 150 950 70 L1000 0 L1000 600 L0 600 Z"/>`;
  const path=pts.length?`<polyline class="map-route" points="${pts.map(p=>{const q=project(p);return `${q.x},${q.y}`}).join(' ')}"/>`:'';
  const nodes=pts.map((p,i)=>{const q=project(p),anchor=q.x>760?'end':'start',dx=q.x>760?-16:16,tip=`${p.name}。${p.summary} 建议停留 ${p.duration} 分钟。${p.slope}。`;return `<g class="map-point" tabindex="0"><title>${esc(tip)}</title><circle class="map-node" cx="${q.x}" cy="${q.y}" r="14"/><text class="map-number" x="${q.x}" y="${q.y+1}">${i+1}</text><text class="map-label" x="${q.x+dx}" y="${q.y-18}" text-anchor="${anchor}">${esc(p.name)}</text></g>`}).join('');
  routeMap.innerHTML=lines.join('')+coast+path+nodes;
}
...
function restore(){const match=location.hash.match(/^#trip=(.+)$/);if(!match)return;if(document.body.classList.contains('home-page')){location.replace(`planner.html${location.hash}`);return}try{const d=JSON.parse(decodeURIComponent(escape(atob(match[1]))));selected=d.p||[];travelDate.value=d.d||travelDate.value;startTime.value=d.t||'09:00';startPlace.value=d.s||'青岛站';transport.value=d.m||'smart';pace.value=d.v||'normal';setTimeout(()=>{renderPlaces();buildPlan();planner.scrollIntoView()},100)}catch{}}

const mapFullscreenButton=$('#mapFullscreen'),mapFullscreenTarget=$('#plannerMapCard');
function setMapExpanded(expanded){if(!mapFullscreenButton||!mapFullscreenTarget)return;mapFullscreenTarget.classList.toggle('map-expanded',expanded);document.body.classList.toggle('map-open',expanded);mapFullscreenButton.setAttribute('aria-expanded',String(expanded));mapFullscreenButton.textContent=expanded?'退出全屏':'全屏查看地图';setTimeout(()=>window.dispatchEvent(new Event('resize')),120)}
if(mapFullscreenButton&&mapFullscreenTarget){mapFullscreenButton.onclick=()=>setMapExpanded(!mapFullscreenTarget.classList.contains('map-expanded'));document.addEventListener('keydown',event=>{if(event.key==='Escape'&&mapFullscreenTarget.classList.contains('map-expanded'))setMapExpanded(false)})}
...
renderRoutes();renderCategoryChart();renderFilters();renderPlaces();fetchWeather();restore();
